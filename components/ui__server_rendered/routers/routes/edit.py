...
from components.ui__server_rendered.routers.routes._helpers import update_from_form
...
def edit_route_form(
    ...
    return templates.TemplateResponse(
        ...
        context={
            "request": request,
            "title": translate(lang, "routes.form_edit_title"),
            "mode": "edit",
...
@router.post("/{route_uuid}/edit")
async def edit_route_ui(
    route_uuid: str,
    request: Request,
    repository: RouteRepositorySqlModel = Depends(get_route_repository),
    office_repository: OfficeRepositorySqlModel = Depends(get_office_repository),
    ...
    if payload is None:
        return templates.TemplateResponse(
            request=request,
            name="routes/form.html",
            context={"request": request, "title": translate(lang, "routes.form_edit_title"), "mode": "edit", "entity_uuid": route.uuid, "form_action": f"/routes/{route.uuid}/edit", "values": values, "errors": errors, "office_options": office_options, "vehicle_options": vehicle_options, "lang": lang},
            status_code=status.HTTP_400_BAD_REQUEST,
        )
    try:
        update_route(repository=repository, route_uuid=route_uuid, **payload.model_dump())
