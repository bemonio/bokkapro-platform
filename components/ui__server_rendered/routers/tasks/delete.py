...
def delete_task_ui(task_id: int, repository: TaskRepositorySqlModel = Depends(get_task_repository), lang: str = Depends(get_locale)):
    try:
        delete_task(repository=repository, task_uuid=task_id)
